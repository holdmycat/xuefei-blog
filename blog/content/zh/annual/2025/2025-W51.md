...
- ✅ **指挥体系分层设计**：完成了 `Commander -> Legion -> Phalanx -> Soldier` 的四层嵌套架构设计，明确了 Zenject Factory (上层逻辑) 与 MemoryPool (海量实体) 的混合策略
- ✅ **中英文档同步**：输出了架构决策文档 `Commander_Unit_Hierarchy.md`，并同步发布至博客

#### 双端 RoomManager 修复

- ✅ **时序逻辑自愈**：修复了 `InitAsync` 中过早创建 Commander 导致的引用丢失问题
- ✅ **对象销毁闭环**：恢复了被误删的 `RpcDestroyObject` 处理逻辑，杜绝了客户端"幽灵单位"
- ✅ **Tick 传播机制**：补全了从 RoomManager 到 Commander 的 `Tick`/`Update` 链路，确保逻辑帧正常驱动
