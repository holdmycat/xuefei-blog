...
### 2.1 美术资源需求 (Art Requirements)

为了平衡展示效果与开发成本，采用**最小化搭配**方案：

| 类别 | 角色 |定位/描述 | 数量 |
| :--- | :--- | :--- | :--- |
| **英雄 (Commanders)** | **盾卫** | **前排**。高血量，技能带护盾/嘲讽。 | 1 |
| | **射手** | **后排**。单体高伤，展示物理弹道。 | 1 |
| | **法师** | **后排**。AOE 伤害，展示绚丽技能特效。 | 1 |
| **兵种 (Legions)** | **盾兵** | 对应前排防御。 | 1 |
| | **骑兵/枪兵** | 对应突击 (Assault)。 | 1 |
| | **弓兵** | 对应后排输出。 | 1 |
| **野怪 (Monsters)** | **杂兵 (Melee)** | 数量多，用于被割草 (如小鬼)。 | 1 |
| | **远程怪 (Ranged)** | 用于给玩家后排压力 (如投掷手)。 | 1 |
| | **Boss** | **第5关限定**。模型巨大，动作帅气 (如巨魔/大骑士)。 | 1 |
| **总计** | | **3 英雄 + 3 兵种 + 3 野怪 = 9 模型** | **9** |

*注：若资源不足，野怪模型可复用兵种模型，仅通过材质变红/光环区分。*

### 2.2 场景美术资源需求 (Scene Art Requirements)

基于参考图（无尽冬日风格），场景需呈现**低多边形 (Low Poly)**、**冷色调**的雪原战场。以下为核心物件清单：

| 物件名称 (Name) | 描述 (Description) | 预估面数 (Tris) |
| :--- | :--- | :--- |
| **Terrain_SnowBase** | **雪地地基**。作为战场主地面，需包含起伏和积雪纹理。 | 500 - 1000 |
| **Terrain_DirtPath** | **泥土路面**。贯穿战场的S型或直线路径，用于引导行军路线。 | 200 - 400 |
| **Env_Cliff_01** | **雪崖/山体**。用于战场边缘阻挡，低多边形切面风格，顶部积雪。 | 300 - 600 |
| **Env_Tree_Pine_Snow** | **积雪松树**。主要植被，层叠状树叶，需有不同大小变体 (L/M/S)。 | 300 - 500 |
| **Env_Tree_Dead** | **枯树干**。无叶枯木，增加战场荒凉感。 | 150 - 300 |
| **Env_Bush_Snow** | **积雪灌木**。点缀地面的小植被。 | 50 - 100 |
| **Env_Rock_Group** | **散落石头组**。不同大小的石头组合，点缀路边或山脚。 | 100 - 200 |
| **VFX_Snow_Drift** | **飘雪粒子**。全局环境特效，营造寒冷氛围。 | (Particle) |
| **VFX_Fog_Plane** | **边缘雾气**。用于隐藏地图边缘的半透明片。 | 2 - 10 |

**风格要求**:

- **材质**: 纯色 (Flat Color) 或 简单渐变 (Gradient)，无复杂法线/高光。
- **色调**: 白色、浅蓝灰、深褐（树干）、土黄（路面）。
