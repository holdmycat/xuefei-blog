...
- **数据格式封闭 (核心痛点)**：
  ...
  - **不可自定义**: 开发者无法干预其底层的序列化 Schema（字段排列、元数据）。
  - **工具链断裂**: 这在战略上是极度封闭的。意味着我们 **完全无法** 使用 Python、Go 或 LLM (ChatGPT) 来读取、分析或批量修改这些资产。
  - *这是通用插件的大忌*：它试图用“通用”来掩盖“封闭”，导致我们失去了对核心资产的掌控权。一旦需要做大规模的 AI 自动化管线（例如用 Python 脚本批量调整怪物数值），Unity 方案将直接成为拦路虎。
